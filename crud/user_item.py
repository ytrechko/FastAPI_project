...
async def link_user_item(user_item: UserItemCreate, db: AsyncSession):
    ...
    if not is_user:
        raise HTTPException(
            status_code=404, detail=f"User with {user_item.id_user} id is not found"
        )
    ...
    if not is_item:
        raise HTTPException(
            status_code=404, detail=f"Item with {user_item.id_item} id is not found"
        )
...
async def update_link_user_item(
    id_user: int, id_item: int, update_user_item: UserItemUpdate, db: AsyncSession
):

    stmt = select(UserItem).where(
        and_(UserItem.id_user == id_user, UserItem.id_item == id_item)
    )
    result = await db.execute(stmt)
    ...
    if not availability_user_with_item:
        raise HTTPException(
            status_code=404,
            detail=f"User {id_user} id  with item {id_item} id is not found",
        )
    ...
    if not availability_item:
        raise HTTPException(
            status_code=404,
            detail=f"Item with {update_user_item.id_item} id is not found",
        )

    stmt = (
        update(UserItem)
        .where(and_(UserItem.id_user == id_user, UserItem.id_item == id_item))
        .values(id_item=update_user_item.id_item)
    )
...
async def delete_user_item(id_user: int, id_item: int, db: AsyncSession):
    ...
    stmt_availability = select(UserItem).where(
        and_(UserItem.id_user == id_user, UserItem.id_item == id_item)
    )
    ...
    stmt_delete = delete(UserItem).where(
        and_(UserItem.id_user == id_user, UserItem.id_item == id_item)
    )
