...
@router.post("/users/")
async def create_user_route(user: UserCreate, db: AsyncSession = Depends(get_db)) -> Dict[str, str]:
    await create_user(user, db)
    return {"status" : "user successful created"}


@router.get("/users/{id_user}", response_model=UserWithItemsRead)
async def read_user_route(id_user: int, db: AsyncSession = Depends(get_db)) -> UserWithItemsRead:
    return await read_user(id_user, db)


@router.get("/users/", response_model=List[UserWithItemsRead])
async def read_all_users_route(db: AsyncSession = Depends(get_db)) -> List[UserWithItemsRead]:
    return await read_all_users(db)


@router.put("/users/{id}/")
async def update_user_route(id: int, user: UserUpdate, db: AsyncSession = Depends(get_db)) -> Dict[str, str]:
    await update_user(id, user, db)
    return {"status" : "user successful updated"}


@router.delete("/users/{id_user}")
async def delete_user_route(id_user: int , db: AsyncSession = Depends(get_db)) -> Dict[str, str]:
